...
var _libForwarders = require('./lib/forwarders');

var _libForwarders2 = _interopRequireDefault(_libForwarders);
...
var core = {
  ...
  buildArgs: function buildArgs() {
    ...
    // Set name
    var name = ['--name', ('devlab_' + core.manifest.workdir.split('/').pop() + '_' + _libConfig2['default'].manifest.username + '_' + _libConfig2['default'].instance).toLowerCase()];
    // From (image) config
   ...
   */
  startForwarders: function startForwarders() {
    ...
    if (!host) {
      return _bluebird2['default'].reject(new Error('DOCKER_HOST is malformed. Cannot start forwarders.'));
    }
    return _libForwarders2['default'].startForwarders(host, ports);
  },
  ...
  run: function run() {
    ...
    // Start
    core.startServices(core.manifest.services).then(core.startForwarders).then(core.buildArgs).then(core.execTask).then(_libForwarders2['default'].stopForwarders).then(_libServices2['default'].stopServices).then(function () {
      ...
      _libForwarders2['default'].stopForwarders();
